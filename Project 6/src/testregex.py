...
class TestRegex(unittest.TestCase):
    ...
    # regex (a|b)* match ""
    def test_clos_alt1(self):
        regex = Closure(Alternation(Character('a'), Character('b')))
        str_to_match = ''
        res = matches(regex, str_to_match)
        assert res == True

    # regex (a|b)* match "a"
    def test_clos_alt2(self):
        regex = Closure(Alternation(Character('a'), Character('b')))
        str_to_match = 'a'
        res = matches(regex, str_to_match)
        assert res == True
    ...
    def test_clos_alt7(self):
        regex = Closure(Alternation(Character('a'), Character('b')))
        str_to_match = 'aaaaaaaabbbbbbbb'
        res = matches(regex, str_to_match)
        print(res)
        assert res == True
